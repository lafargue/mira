...
const ISO_CWD = mkdtempSync(join(tmpdir(), "grok-iso-"));
const iso = (ctx = {}) => ({ cwd: ISO_CWD, site: {}, ...ctx });
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, iso({ appName: "Wild Race" }));
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", iso({
    host: "wild-race.grok.me",
  }));
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectGrokPwaHead("<html><head></head></html>", iso({
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  }));
  ...
  const custom = injectGrokPwaHead("<html><head></head></html>", iso({
    appName: "Wild Race",
    host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead("<html><head></head></html>", iso({
    host: "wild-race.grok.me",
  ...
  const invalid = injectGrokPwaHead("<html><head></head></html>", iso({
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", iso({ appName: "Solo" }));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(iso({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", iso({ appName: "Wild Race" }));
